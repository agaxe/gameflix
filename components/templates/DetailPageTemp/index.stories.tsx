...
export function detailPageTemp() {
  ...
  const aggregated_rating =
    aggregated_rating_state &&
    number('게임 평점', dummyGames.aggregated_rating);
  ...
  const involved_companies_text =
    involved_companies_state &&
    text('게임 제작 회사 ', dummyGames.involved_companies[0].company.name);
  ...
  const videosLength = number(`비디오 수 ( 최대 ${videosMax} )`, videosMax, {
    min: 0,
    max: videosMax
  });
  const screenshotsLength = number(
    `스크린샷 수 ( 최대 ${screenshotsMax} )`,
    screenshotsMax,
    { min: 0, max: screenshotsMax }
  );
  ...
  const mediaFunc = (type: 'videos' | 'screenshots', length) => {
    if (!length) {
      ...
      if (type === 'screenshots' && length > screenshotsMax)
        maxNum = screenshotsMax;
  ...
  const detailData = {
    ...
    first_release_date:
      first_release_date_state && dummyGames.first_release_date,
    aggregated_rating,
    summary,
    involved_companies,
  ...
  return (
    <div>
      <DetailPageTemp
        data={detailData}
        similarGames={similarGameState ? similarGameData : []}
        recentGames={recentGamesDataState ? recentGamesData : []}
      />
    </div>
  );
}
