...
import { SectionTitle, GameCard } from 'components/molecules';
...
function DetailPageTemp({
	...
	const companyName = (involved_companies) ? involved_companies[0].company.name : null;
	const genresList = (genres) ? arrayToString(genres.map(item => item.name)) : null;
	const platformsList = (platforms) ? arrayToString(platforms.map(item => item.name)) : null;
	...
	const webSiteText = (category) => {
		...
		const categoryInfo = categoryList.filter(item => +item.category === category)
	...
	const noGameInfo = (type: string) => {
		return <NoGameInfoText><p>{type} 정보가 없습니다 :(</p></NoGameInfoText>
	}
			...
			<SectionBox>
				<InfoBox>
					...
					<InfoText>
						<h3>{name}</h3>
						<p>
							{timeStampToDate(first_release_date)}
						</p>
						<strong>
							{aggregated_rating
									...
									<GameSummary summaryMore={summaryMore}>
										{
											!summary
												? noGameInfo('설명')
												: (summary && !summaryMore && (summary.length > summaryShort.length))
													? <>
														<p>{summaryShort}...</p>
														<span onClick={() => setSummaryMore(true)}>
															more
														</span>
													</>
													: <p>{summary}</p>
									...
									<AgeRatingList>
										{age_ratings
											? age_ratings.map((item, idx) => (
												<Item key={idx}>
													<img
														src={`/static/images/game_rating/${ageRatingImg(item.category, item.rating)}`}
			...
			</SectionBox>
			{
				mediaContents.length
					? <SectionBox>
						...
						<MediaSlideBox>
							<MediaSlide
								css={{ 'margin-left': (mediaContents.length >= 3) ? '-220px' : '0' }}
									...
									mediaContents.map((item, idx) => (
										<SwiperSlide key={idx} className={`${idx}`}>
											<MediaSlideContent
												...
												css={{
													"background": (item.image_id && !item.video_id)
						...
						</MediaSlideBox>
						{
							...
							<>
								...
								<MediaModal>
									<MediaModalContent>
										{(mediaContents.length !== 1) && <MdChevronLeft onClick={prevModalContent} />}
										<div>
											{
										...
										{(mediaContents.length !== 1) && <MdChevronRight onClick={nextModalContent} />}
									</MediaModalContent>
								</MediaModal>
							</>
						}
				...
				<RecentGameList>
					{
						recentGamesData.length
							...
							: [...Array(5)].map((item, idx) => <GameCard key={idx} skeleton={true} />)
					}
				</RecentGameList>
			</SectionBox>
		</section>
