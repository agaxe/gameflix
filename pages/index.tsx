...
export async function getServerSideProps() {
	...
	const streamListFunc = async () => {
		...
		// 상위 5위 게임
		const gameTop = await useTwitchApi('games/top?first=5')
			.then(res => res.data.filter((item, idx) => item.id !== "509658" && idx < 4));

		// 해당 게임의 스트리머 정보 ( 아이디 x )
		let gameStreamer =
			await gameTop.map(async item => {
			...
			gameTop.map(async (game, index) => {
				const streamers = await Promise.all(
					gameStreamer[index].map((item, idx) => {
						return useTwitchApi(`users?id=${item['user_id']}`)
							.then(res => {
