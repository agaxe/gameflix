...
import { getTwtichData } from '@/utils/getTwtichData';
...
export async function getServerSideProps() {
  ...
  try {
    ...
    const getStreamsData = async () => {
      ...
      const gameTop3 = await getTwtichData('games/top?first=5').then((res) =>
        res.data.filter((item, idx) => item.id !== justChattingId && idx <= 3)
      );

      return await Promise.all(
        gameTop3.map(async (game, index) => {
          const streams = await getTwtichData(
          ...
          const streamers = await Promise.all(
            streams.data.map(async (stream) => {
              ...
              const user = await getTwtichData(`users?id=${stream['user_id']}`);
              const { id, login, display_name, profile_image_url } =
                user.data[0];
    ...
    return {
      props: { popularGames, comingSoonGames, liveGameStreams }
    };
  } catch (error) {
    return {
      notFound: true
